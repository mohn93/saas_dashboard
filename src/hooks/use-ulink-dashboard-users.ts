...
interface UseULinkDashboardUsersResult {
...
export function useULinkDashboardUsers(
  start: string,
  end: string
): UseULinkDashboardUsersResult {
  ...
  useEffect(() => {
    ...
    fetch(`/api/metrics/ulink/dashboard-users?${params}`)
      .then((res) => res.json())
      ...
      .catch((err) => {
        if (cancelled) return;
        setError(err.message || "Failed to fetch dashboard users metrics");
      })
      .finally(() => {
