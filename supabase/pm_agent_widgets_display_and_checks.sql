...
-- drop-then-add keeps the script idempotent (safe to re-run).
alter table pm_agent.widgets drop constraint if exists widgets_display_chk;
alter table pm_agent.widgets
  add constraint widgets_display_chk check (display in ('table', 'chart', 'both'));
alter table pm_agent.widgets drop constraint if exists widgets_kind_chk;
alter table pm_agent.widgets
  add constraint widgets_kind_chk check (kind in ('chart', 'table', 'kpi', 'text'));
alter table pm_agent.widgets drop constraint if exists widgets_size_chk;
alter table pm_agent.widgets
